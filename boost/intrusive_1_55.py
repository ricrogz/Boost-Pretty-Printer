...
class Hook_Printer:
    ...
    def to_string(self):
        return '<content hidden>' if options.get('hide_intrusive_hooks',
                                                 False) else None
    ...
    if template_name(template_arg_t).startswith('boost::intrusive::get_'):
        ...
        for case in switch(template_name(template_arg_t)):
            if case('boost::intrusive::get_list_node_algo'):
                return lookup_type('boost::intrusive::list_node_traits<' +
                                   str(voidptr_t) + '>')
            if case('boost::intrusive::get_slist_node_algo'):
                return lookup_type('boost::intrusive::slist_node_traits<' +
                                   str(voidptr_t) + '>')
            if case('boost::intrusive::get_set_node_algo'):
                opt_size = template_arg_t.template_argument(1)
                return lookup_type('boost::intrusive::rbtree_node_traits<' +
                                   str(voidptr_t) + ',' + str(opt_size) + '>')
            if case('boost::intrusive::get_avl_set_node_algo'):
                opt_size = template_arg_t.template_argument(1)
                return lookup_type('boost::intrusive::avltree_node_traits<' +
                                   str(voidptr_t) + ',' + str(opt_size) + '>')
            if case('boost::intrusive::get_bs_set_node_algo'):
                return lookup_type('boost::intrusive::tree_node_traits<' +
                                   str(voidptr_t) + '>')
    else:
...
@add_to_dict(static_method,
             ('boost::intrusive::trivial_value_traits', 'to_value_ptr'))
def f(vtt, node_rptr):
    return node_rptr
...
def f(vtt, node_rptr):
    ...
    def get_hook_type(value_t, tag_t):
        ...
        value_base_types = (field.type for field in value_t.fields()
                            if field.is_base_class)
        for value_base_type in value_base_types:
            # Checking if base_type is a hook
            for field in value_base_type.fields():
                if field.is_base_class and template_name(
                        field.type) == 'boost::intrusive::generic_hook':
        ...
        assert False, 'no subclass hook with tag: ' + str(
            tag_t.strip_typedefs())
...
#
@add_to_dict(static_method, ('boost::intrusive::list_node_traits', 'get_next'),
...
class Iterator_Printer:
    ...
    max_supported_version = (1, 69, 0)
    template_name = [
        'boost::intrusive::list_iterator', 'boost::intrusive::slist_iterator',
    ...
    def children(self):
        ...
        node_rptr = unwind_references(
            call_object_method(self.val, 'pointed_node'))
        value_rptr = get_raw_ptr(
            call_static_method(value_traits_t, 'to_value_ptr', node_rptr))
...
class List_Printer:
    ...
    class Iterator:
        ...
        def __init__(self, v):
            ...
            self.root_node_rptr = get_raw_ptr(
                call_object_method(v, 'get_root_node'))

        def __iter__(self):
            self.count = 0
            self.crt_node_rptr = get_raw_ptr(
                call_static_method(self.node_traits_t, 'get_next',
                                   self.root_node_rptr))
            return self

        def __next__(self):
            if self.crt_node_rptr == self.root_node_rptr or is_null(
                    self.crt_node_rptr):
                raise StopIteration
            val_rptr = get_raw_ptr(
                call_static_method(self.value_traits_t, 'to_value_ptr',
                                   self.crt_node_rptr))
    ...
    def __init__(self, v):
        ...
        self.v.node_traits_t = get_inner_type(self.v.list_impl_t,
                                              'node_traits')
...
class Tree_Printer:
    ...
    def get_bstree_impl_base(t):
        ...
        #
        t_list = list()
        while (len(t_list) < 5 and isinstance(t, gdb.Type)
               and t.code == gdb.TYPE_CODE_STRUCT
            ...
            try:
                ...
                return None
        if isinstance(t, gdb.Type) and template_name(
                t) == 'boost::intrusive::bstree_impl':
            return t
        else:
            return None
    ...
    class Iterator:
        ...
        def __init__(self, v):
            ...
            if template_name(self.node_traits_t) in [
                    'boost::intrusive::avltree_node_traits',
                    'boost::intrusive::rbtree_node_traits'
            ]:
                self.optimize_size = bool(
                    self.node_traits_t.template_argument(1))
            self.header_node_rptr = get_raw_ptr(
                call_object_method(v.cast(v.bstree_impl_t), 'header_ptr'))

        def __iter__(self):
            self.count = 0
            self.crt_node_rptr = get_raw_ptr(
                call_static_method(self.node_traits_t, 'get_left',
                                   self.header_node_rptr))
            return self
        ...
        def advance(self):
            n = get_raw_ptr(
                call_static_method(self.node_traits_t, 'get_right',
                                   self.crt_node_rptr))
            if not is_null(n):
                ...
                while True:
                    ...
                    self.crt_node_rptr = get_raw_ptr(
                        call_static_method(self.node_traits_t, 'get_parent',
                                           self.crt_node_rptr))
                    if self.optimize_size:
                        self.crt_node_rptr = parse_and_eval(
                            '(' +
                            str(get_basic_type(self.crt_node_rptr.type)) +
                            ')(((size_t)' +
                            str(self.crt_node_rptr).split()[0] +
                            ') & (~(size_t)3))')
                    if self.crt_node_rptr == self.header_node_rptr:
                        break
                    n = get_raw_ptr(
    ...
    def __init__(self, v):
        ...
        self.v.node_traits_t = get_inner_type(self.v.bstree_impl_t,
                                              'node_traits')
