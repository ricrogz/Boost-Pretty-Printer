...
from boost.variant import apply_qualifiers, strip_qualifiers
...
def execute_cpp_function(function_name):
    ...
    bp = gdb.Breakpoint(function='dummy_function',
                        internal=True,
                        temporary=True)
    bp.silent = True
...
def to_python_value(value):
    ...
    if type.code == gdb.TYPE_CODE_STRUCT:
        return {
            name: to_python_value(value[field])
            for name, field in gdb.types.deep_items(type)
...
def as_map(children_values,
           key_func=to_python_value,
           value_func=to_python_value):
    ...
    return [(key_func(key), value_func(value))
            for ((key_text, key), (value_text, value)) in zip(it, it)]
...
def run_printer_tests(module_contents):
    ...
    test_cases = [
        obj for obj in module_contents
        if inspect.isclass(obj) and issubclass(obj, PrettyPrinterTest)
        and obj is not PrettyPrinterTest and test_re.search(obj.__name__)
    ]
...
boost_version = boost.detect_version.unpack_boost_version(
    int(gdb.parse_and_eval('boost_version')))
...
class IteratorRangeTest(PrettyPrinterTest):
    ...
    def test_char_range(self):
        ...
        self.assertEqual(display_hint, 'array')
        self.assertEqual(
            as_array(children, lambda v: chr(int(v))),
...
@unittest.skipIf(boost_version < (1, 55),
                 'implemented in boost 1.55 and later')
...
@unittest.skipIf(boost_version < (1, 58),
                 'implemented in boost 1.58 and later')
...
@unittest.skipIf(boost_version < (1, 58),
                 'Printer was implemented for boost 1.58 and later versions')
...
class VariantTest(PrettyPrinterTest):
    ...
    def check_type(self, var_name):
        ...
        recovered_type = apply_qualifiers(gdb.lookup_type(base_type_name),
                                          qualifiers)
    ...
    def test_variant_ts(self):
        ...
        self.assertEqual(
            string, '(boost::variant<...>) type = VariantTs<int, int, int>')
        self.assertEqual(as_struct(children),
...
@unittest.skipIf(boost_version < (1, 42, 0),
                 'implemented in boost 1.42 and later')
class UuidTest(PrettyPrinterTest):
    ...
    def test_uuid(self):
        ...
        self.assertEqual(
            string,
            '(boost::uuids::uuid) 01234567-89ab-cdef-0123-456789abcdef')
        self.assertIsNone(children)
        self.assertIsNone(display_hint)
...
class DateTimeTest(PrettyPrinterTest):
    ...
    def test_posinfin_time(self):
        ...
        self.assertEqual(string,
                         '(boost::posix_time::ptime) positive infinity')
        self.assertIsNone(children)
        self.assertIsNone(display_hint)
    ...
    def test_neginfin_time(self):
        string, children, display_hint = self.get_printer_result(
            'neg_infin_time')
        self.assertEqual(string,
                         '(boost::posix_time::ptime) negative infinity')
        self.assertIsNone(children)
        self.assertIsNone(display_hint)
...
@unittest.skipIf(boost_version < (1, 48, 0),
                 'implemented in boost 1.48 and later')
class FlatSetTest(PrettyPrinterTest):
    ...
    def test_empty_set(self):
        ...
        self.assertEqual(string,
                         'boost::container::flat_set<int> size=0 capacity=0')
        self.assertEqual(children, [])
        self.assertEqual(display_hint, 'array')
    ...
    def test_full_set(self):
        ...
        self.assertEqual(string,
                         'boost::container::flat_set<int> size=2 capacity=4')
        self.assertEqual(as_array(children), [1, 2])
        self.assertEqual(display_hint, 'array')
...
class FlatMapTest(PrettyPrinterTest):
    ...
    def test_empty_map(self):
        ...
        self.assertEqual(
            string, 'boost::container::flat_map<int, int> size=0 capacity=0')
        self.assertEqual(children, [])
    ...
    def test_map_full(self):
        ...
        self.assertEqual(
            string, 'boost::container::flat_map<int, int> size=2 capacity=4')
...
@unittest.skipUnless((1, 55, 0) <= boost_version < (
    1, 70, 0
), 'Tests for intrusive containers are not supported for boost < 1.55 or boost >= 1.70'
                     )
class IntrusiveBaseSetTest(PrettyPrinterTest):
    ...
    def test_base_set_1(self):
        ...
        self.assertEqual(string, None)
        self.assertEqual(as_array(children, lambda val: int(val['int_'])),
                         [1, 2, 3])
        self.assertEqual(display_hint, 'array')
...
class IntrusiveMemberRbtreeSetTest(PrettyPrinterTest,
                                   IntrusiveMemberSetCommon):
...
class IntrusiveMemberAvlTreeSetTest(PrettyPrinterTest,
                                    IntrusiveMemberSetCommon):
...
class IntrusiveMemberSplayTreeSetTest(PrettyPrinterTest,
                                      IntrusiveMemberSetCommon):
...
class IntrusiveMemberSgTreeSetTest(PrettyPrinterTest,
                                   IntrusiveMemberSetCommon):
...
class UnorderedMapTest(PrettyPrinterTest):
    ...
    def test_empty_map(self):
        ...
        self.assertEqual(
            string,
            'boost::unordered::unordered_map<int, const char *> size = 0')
        self.assertEqual(children, [])
        self.assertEqual(display_hint, 'map')
    ...
    def test_map(self):
        ...
        self.assertEqual(
            'boost::unordered::unordered_map<int, const char *> size = 3',
            string)
        self.assertEqual(sorted(as_map(children)), self.expected_content)
        self.assertEqual('map', display_hint)

    def test_big_map(self):
        string, children, display_hint = self.get_printer_result('big_map')
        self.assertEqual(
            'boost::unordered::unordered_map<int, int> size = 100000', string)
    ...
    def test_iter(self):
        ...
        } for key, value in self.expected_content]
...
class UnorderedMultimapTest(PrettyPrinterTest):
    ...
    expected_content = [(10, 'dieci'), (10, 'ten'), (20, 'twenty'),
                        (20, 'venti'), (30, 'thirty'), (30, 'trenta')]

    def test_empty_multimap(self):
        string, children, display_hint = self.get_printer_result('empty_map')
        self.assertEqual(
            string,
            'boost::unordered::unordered_multimap<int, const char *> size = 0')
        self.assertEqual(children, [])
        self.assertEqual(display_hint, 'map')
    ...
    def test_multimap(self):
        string, children, display_hint = self.get_printer_result('map')
        self.assertEqual(
            'boost::unordered::unordered_multimap<int, const char *> size = 6',
            string)
        self.assertEqual(sorted(as_map(children)), self.expected_content)
        self.assertEqual('map', display_hint)
...
class UnorderedSetTest(PrettyPrinterTest):
    ...
    def test_empty_set(self):
        string, children, display_hint = self.get_printer_result('empty_set')
        self.assertEqual(
            string, 'boost::unordered::unordered_set<const char *> size = 0')
        self.assertEqual(children, [])
        self.assertEqual(display_hint, 'array')
    ...
    def test_set(self):
        ...
        self.assertEqual(
            'boost::unordered::unordered_set<const char *> size = 3', string)
        self.assertEqual(sorted(as_array(children)), self.expected_content)
...
class UnorderedMultisetTest(PrettyPrinterTest):
    ...
    def test_empty_multiset(self):
        ...
        self.assertEqual(
            string,
            'boost::unordered::unordered_multiset<const char *> size = 0')
        self.assertEqual(children, [])
        self.assertEqual(display_hint, 'array')
    ...
    def test_multiset(self):
        ...
        self.assertEqual(
            'boost::unordered::unordered_multiset<const char *> size = 4',
            string)
        self.assertEqual(sorted(as_array(children)), self.expected_content)
        self.assertEqual('array', display_hint)
...
class DurationTest(PrettyPrinterTest):
    ...
    def test_duration_with_ms(self):
        string, children, display_hint = self.get_printer_result(
            'duration_with_ms')
        self.assertEqual(string,
                         '(boost::posix_time::time_duration) 1m 1.010000s')
        self.assertEqual(children, None)
        self.assertEqual(display_hint, None)
...
@unittest.skipIf(boost_version < (1, 56, 0),
                 'implemented in boost 1.56 and later')
class MultiIndexTest(PrettyPrinterTest):
    ...
    def test_hashed_first_over2_same_value(self):
        string, children, display_hint = self.get_printer_result(
            'hf_over_two_same_value')
        self.assertEqual(sorted(as_array(children, int)),
                         [1, 1, 1, 2, 2, 2, 2, 3, 3, 4])  # unordered
        self.assertIsNone(display_hint)


@unittest.skipIf(boost_version < (1, 71),
                 'implemented in boost 1.71 and later')
class WaveTest(PrettyPrinterTest):
    ...
    def test_token(self):
        ...
        self.assertEqual(
            string, 'boost::wave::T_IDENTIFIER : "data" ("<Unknown>" 2:21)')
        self.assertIsNone(children)
        self.assertIsNone(display_hint)
